...
public class testPlayer : Character
{
    ...
    private void CrouchUp(float direction)
    {
        ...
        if (crouching)
        {
            ...
            underPlattform = Physics2D.Raycast((Vector2)transform.position, transform.up, plattformCheckDistance, whatIsPlatform);
            ...
            if (!underPlattform)
            {
                ...
                inFrontOfPlayer = Physics2D.Raycast((Vector2)transform.position + new Vector2(0.3f, 0f), transform.up, plattformCheckDistance, whatIsPlatform);
                behindPlayer = Physics2D.Raycast((Vector2)transform.position + new Vector2(-0.3f, 0f), transform.up, plattformCheckDistance, whatIsPlatform);
